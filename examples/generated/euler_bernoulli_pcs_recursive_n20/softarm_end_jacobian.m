...
%#codegen
J=softarm_mount_jacobian(q,p); T=softarm_mount_transform(q,p); zeroDq=zeros(40,1);
for section=1:20
    ...
    localJv=reshape(jet(cursor:cursor+5),3,2); cursor=cursor+6;
    localJw=reshape(jet(cursor:cursor+5),3,2);
